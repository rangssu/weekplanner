...
import {
  BORDER_WIDTH, CANVAS_BORDER_WIDTH, CANVAS_CONTENT_HEIGHT, CANVAS_CONTENT_WIDTH,
  CANVAS_HEIGHT, CANVAS_WIDTH, CELL_HEIGHT, CELL_TEXT_HEIGHT, CELL_TEXT_WIDTH,
  CELL_WIDTH, DOW_ROW_HEIGHT, GRID_AREA_HEIGHT, GRID_AREA_WIDTH,
  GRID_INNER_HEIGHT, GRID_INNER_WIDTH, HEADER_HEIGHT, OUTER_PADDING,
} from './layout'
...
describe('레이아웃 상수', () => {
  ...
  it('세로 구성 요소의 합이 캔버스 높이와 정확히 같다', () => {
    const total = CANVAS_BORDER_WIDTH * 2 + OUTER_PADDING * 2 + HEADER_HEIGHT + GRID_AREA_HEIGHT
    expect(total).toBe(CANVAS_HEIGHT)
  })

  it('가로 구성 요소의 합이 캔버스 폭과 정확히 같다', () => {
    expect(CANVAS_BORDER_WIDTH * 2 + OUTER_PADDING * 2 + GRID_AREA_WIDTH).toBe(CANVAS_WIDTH)
  })

  it('캔버스 테두리를 빼먹지 않았다 — 여백이 실제로 OUTER_PADDING만큼 남는다', () => {
    expect(CANVAS_CONTENT_WIDTH).toBe(CANVAS_WIDTH - CANVAS_BORDER_WIDTH * 2 - OUTER_PADDING * 2)
    expect(CANVAS_CONTENT_HEIGHT).toBe(CANVAS_HEIGHT - CANVAS_BORDER_WIDTH * 2 - OUTER_PADDING * 2)
  })
