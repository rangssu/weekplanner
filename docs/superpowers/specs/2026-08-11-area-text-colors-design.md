...
CSS `transition-*`는 **상속되지 않는다.** `ScheduleCanvas`의 루트에 걸어도 루트 자신의 `color`는 `theme.bodyText`이고 이 기능으로 안 바뀐다. 실제로 색이 바뀌는 요소들(`TitleBar`, `SidebarBox`, `Sidebar`, `DayCell`, `AutoFitText`)은 각자 자기 `color`를 인라인으로 직접 쓰는데, 자손 각각에 `transition`을 거는 것은 별도 작업이다.

그 경로를 택하더라도 내보내기 직전에 끄는 방어(원래 7장의 대책)를 **자손 전체로** 넓혀야 한다 — 루트 하나만 끄면 실제로 전환 중인 자손에는 안 닿는다. 비용이 값어치를 넘는다고 판단해 걷어냈다.
...
7. 배경을 바꾸자마자 곧바로 내보내기를 눌러도 결과가 올바른지
...
| 상자 힌트 문구 | 비어 있을 때만 보이는 안내다. 상자가 비어 있으면 이것만 화면에 남으므로 영역 글자색을 흐리게(`withAlpha(textColor, 0.55)`) 써서 자동 판정을 따라가게 한다 |
| 배지 글자 | 배지 배경도 `bgOpacity`로 함께 투명해지므로(`withAlpha(theme.dowHeaderBackground, bgOpacity)`) 영역 글자색을 그대로 쓴다 |
| 요일 헤더 글자 | 자기 배경색(`dowHeaderBackground`) 위에 있고 이 기능의 5영역에 안 들어 제외 |
