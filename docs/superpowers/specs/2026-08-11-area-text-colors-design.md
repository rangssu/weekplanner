...
**고르는 규칙은 대칭이다. 원래 색이 그 배경에서 읽히면 그대로 쓰고, 안 읽힐 때만 극값으로 바꾼다.**

```
원래 색 = 제목이면 theme.headerText, 나머지는 theme.bodyText

밝은 배경(tone === 'dark')
    원래 색이 임계치보다 어두우면  → 원래 색
    아니면                        → theme.autoTextOnLight

어두운 배경(tone === 'light')
    원래 색이 임계치보다 밝으면    → 원래 색
    아니면                        → theme.autoTextOnDark
```

**한 값만 두면 다크 테마에서 깨진다.** 「밝은 배경이면 기존 테마색」으로만 하면 다크 테마의 `bodyText`가 이미 밝은 색이라 밝은 사진 위에 밝은 글자가 나온다. 양쪽 극이 다 필요하다.

**「원래 색을 먼저 본다」가 중요한 이유** — `headerText`와 `bodyText`가 테마마다 같지 않다. 화이트 테마는 제목이 `#18181b`, 본문이 `#27272a`이고 다크 테마도 갈라진다. 극값 하나로 밀어붙이면 밝은 사진을 깔았을 때 **제목 색이 미묘하게 바뀐다.** 원래 색을 먼저 시험하면 읽히는 한 그대로 남아, 2.1절의 「배경이 없으면 테마 기본색」과 자연스럽게 이어진다.

`autoTextOnLight`·`autoTextOnDark`는 **원래 색이 못 버틸 때만 쓰이는 대비책**이다. 밝은 테마 4종은 `autoTextOnDark`만 실제로 쓰이고, 다크 테마는 `autoTextOnLight`만 쓰인다. 그래도 양쪽을 다 채워 둔다 — 나중에 테마 색을 손볼 때 어느 쪽이 필요해질지 모른다.
...
전환이 실제로 값어치를 하는 곳은 배경을 바꿀 때가 아니라 **불투명도 슬라이더를 끌 때**다. 실효 밝기가 임계치(140) 근처를 오가면 글자색이 톡톡 튄다. 전환이 그 깜빡임을 눌러 준다. 배경 교체 때는 이미지가 어차피 즉시 바뀌므로 티가 안 난다.

**위험**: `html-to-image`는 캡처 시점에 `getComputedStyle`을 읽는다. 전환이 진행 중이면 **중간 색이 PNG에 박힌다.** 실제로 그 순간에 내보내기를 누를 확률은 낮지만(전환 150~200ms vs 사람이 버튼을 누르는 시간), `preview/`가 「화면에 보이는 것이 결과물 그 자체」라는 규칙 위에 서 있으므로 확률에 기대지 않는다.

**대책**: `exportImage.ts`가 캡처 전후로 전환을 껐다 켠다.

```
캡처 전   node.style.transition = 'none'
          (진행 중이던 전환이 취소되고 목표 색으로 즉시 확정된다)
캡처
캡처 후   원래 값으로 되돌린다
```

CSS 명세상 전환을 없애면 진행 중인 전환이 취소되고 변경 후 스타일이 곧바로 계산값이 된다. 중간 색이 박힐 경로가 사라진다. **약 5줄이고 `exportImage.ts` 한 곳만 건드린다.**

전환 시간은 **150ms**로 짧게 잡는다. 편집 패널의 색상 견본에도 같이 넣는다 — `editor/`에는 이 제약이 없다.
...
| `exportImage.test.ts` | 캡처하는 동안 캔버스의 `transition`이 `'none'`이고, 끝난 뒤 원래 값으로 돌아옴 (7장) |
...
7. **배경을 바꾸자마자 곧바로 내보내기를 눌러 중간 색이 안 박히는지** (7장의 대책이 실제로 도는지)
8. 불투명도 슬라이더를 임계치 근처에서 천천히 끌어 글자색이 부드럽게 넘어가는지
